...
int CRTPReceive::init()
{
	...
	outputFile.push_back("output1.wav");
	outputFile.push_back("output2.wav");
	outputFile.push_back("output3.wav");
	string temp;
	...
	for (int i = 0; i < tracks; ++i)
	{
		if (open_input_file(audioPathStr[i], &ifcx[i], &iccx[i]) < 0){
			av_log(NULL, AV_LOG_ERROR, "Error while opening file 1\n");
			exit(3);
		}
		av_dump_format(ifcx[i], 0, audioPathStr[i].c_str(), 0);
	}
...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
int CRTPReceive::init_filter_graph()
{
	...
	while (nb_finished < tracks) 
	{
		...
		if (data_present_in_graph){
			...
			while (1) 
			{
				...
				{
					ret = encode_audio_frame(filt_frame, out_ifcx[j], out_iccx[j], &data_present);
					if (ret < 0){ std::cout << "\n 2.6"; goto end; }
				}
