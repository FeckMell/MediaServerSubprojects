...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
int CRTPReceive::init(vector<string> SDPs)
{
	int err;
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		err = open_output_file(outputFile[i], iccx[i], &out_ifcx[i], &out_iccx[i]);
		//err = open_output_file2(outputFile[i], iccx[i], out_ifcx[i], out_iccx[i]);
...
*/
int CRTPReceive::open_output_file(const char *filename, AVCodecContext *input_codec_context,
...
int CRTPReceive::open_output_file2(const char *filename, AVCodecContext *input_codec_context,
	AVFormatContext *output_format_context, AVCodecContext *output_codec_context)
{
	avformat_alloc_output_context2(&output_format_context, nullptr, "rtp", filename);
	cout << "\n--OPENRTP BIND END";
	...
	auto strmOut = avformat_new_stream(output_format_context, output_codec);
	strmOut->time_base = { 1, 8000 };
	output_codec_context = strmOut->codec;
