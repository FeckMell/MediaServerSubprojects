...
int decode_audio_frame(AVFrame *frame,
	AVFormatContext *input_format_context,
	AVCodecContext *input_codec_context,
	int *data_present, bool *finished)
{
...
void testConf()
{
	const string strOutAddr = getSelfIP();
	...
	FIN_OUT finout[] = {
		...
		{ "in1.sdp", { strOutAddr, 6000, 6100, 20 } }
		, { "in2.sdp", { strOutAddr, 7000, 7100, 20 } }
		, { "in3.sdp", { strOutAddr, 8000, 8100, 20 } }
		, { "in4.sdp", { strOutAddr, 9000, 9100, 20 } }
	};
	for (auto& entry : finout)
		room.newPoint(entry.file, entry.rtpdest);
...
void testConfAsio()
{
	...
	CConfRoom room(io_service);
	IN_OUT inout[] = {
		{ "in1.sdp", 10000, { strOutAddr, 6000, 6100, 0 } }
		, { "in2.sdp", 20000, { strOutAddr, 7000, 7100, 0 } }
		, { "in3.sdp", 30000, { strOutAddr, 8000, 8100, 0 } }
		, { "in4.sdp", 40000, { strOutAddr, 9000, 9100, 20 } }
	};
	for (auto& entry : inout)
		room.newPoint(entry.sdp_file, entry.potIn, entry.rtpdest);
...
{
	cout << "testConfAsio();";
	//testConfAsio(); //Конференция с использованием входящих RTP через boost::asio 
	testConf();  //Конференция с использованием входящих RTP через ffmpeg 
	
	system("pause");
	//testAmix(); //Микширование в CDestFusion
	//testAmixOrigin(); //Микширование - пример из инета
