...
//static const AVRational av_get_time_base_q() = { 1, AV_TIME_BASE };
...
static void close_output_stream(OutputStream *ost)
{
	...
	if (of->shortest) {
		int64_t end = av_rescale_q(ost->sync_opts - ost->first_pts, ost->enc_ctx->time_base, av_get_time_base_q());
...
static void do_subtitle_out(AVFormatContext *s,OutputStream *ost,InputStream *ist,AVSubtitle *sub)
{
	...
	for (i = 0; i < nb; i++) {
		...
		pkt.duration = av_rescale_q(sub->end_display_time, av_get_time_base_q(), ost->st->time_base);
		if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
...
static void do_video_out(AVFormatContext *s,OutputStream *ost,AVFrame *next_picture,double sync_ipts)
{
	...
	std::memmove(ost->last_nb0_frames + 1,
		ost->last_nb0_frames,
		...
		while (1) {
			...
			if (filtered_frame->pts != AV_NOPTS_VALUE) {
				...
				float_pts =
					...
					av_rescale_q(start_time, av_get_time_base_q(), tb);
				...
				filtered_frame->pts =
					...
					av_rescale_q(start_time, av_get_time_base_q(), enc->time_base);
			}
			//if (ost->source_index >= 0)
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
	...
	if (!ost->frame_number && !ost->copy_prior_start) {
		...
		if (pkt->pts == AV_NOPTS_VALUE ?
			ist->pts < comp_start :
			pkt->pts < av_rescale_q(comp_start, av_get_time_base_q(), ist->st->time_base))
			return;
	}
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
	...
	if (decoded_frame->pts != AV_NOPTS_VALUE)
		...
		/*(AVRational){1, avctx->sample_rate}*/av_get_time_base_q(), decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last,
		/*(AVRational){1, avctx->sample_rate}*/av_get_time_base_q());
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
	...
	if (best_effort_timestamp != AV_NOPTS_VALUE) {
		int64_t ts = av_rescale_q(decoded_frame->pts = best_effort_timestamp, ist->st->time_base, av_get_time_base_q());
			...
			case AVMEDIA_TYPE_VIDEO:
				...
				if (ost->frame_aspect_ratio.num) { // overridden by the -aspect cli option
					sar =
						...
						/*(AVRational){enc_ctx->height, enc_ctx->width}*/av_get_time_base_q());
...
static int process_input(int file_index)
{
	...
	if (debug_ts) {
		/*av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
			...
			av_ts2str(ist->next_dts), av_ts2timestr(ist->next_dts, &av_get_time_base_q()),
			av_ts2str(ist->next_pts), av_ts2timestr(ist->next_pts, &av_get_time_base_q()),
			av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
			av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
			av_ts2str(input_files[ist->file_index]->ts_offset),
			av_ts2timestr(input_files[ist->file_index]->ts_offset, &av_get_time_base_q()));*/
	}
	...
	pkt_dts = av_rescale_q_rnd(pkt.dts, ist->st->time_base, av_get_time_base_q(), AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
