	...
	AVCodecContext *input_codec_context, int *data_present, int *finished, int i)
{
	...
	// putting incoming data in file, to check, that its came ok
	if (i == 0)
	{
		...
		for (int i = 0; i < tracks; ++i)
		{
			...
			*/
			if (finished && !data_present)
			{
				...
				//for every client giving NULL frames to buffers associeted with him
			...
			else if (data_present)
			{
				...
				//каждому клиенту в соответствующий буффер даем фрейм
				//for every client giving frames to buffers associeted with him
				for (int j = 0; j < tracks; ++j) //вернуть
				{
					if (i == j) continue;
