...
int CRTPReceive::init(vector<string> SDP)
{
	...
	for (int i = 0; i < tracks; ++i)
	{
		...
		av_dump_format(ifcx[i], 0, SDP[i].c_str(), 0);
	}
	for (int i = 0; i < tracks; ++i)
	{
		err = init_filter_graph(i);
		if (err < 0) std::printf("Init err = %d\n", err);
	...
	AVFormatContext **output_format_context, AVCodecContext **output_codec_context)
{
	...
	if (!(output_codec = avcodec_find_encoder((*input_format_context)->streams[0]->codec->codec_id)/*AV_CODEC_ID_PCM_S16LE)*/)) {
...
int CRTPReceive::encode_audio_frame(AVFrame *frame, AVFormatContext *output_format_context,
	AVCodecContext *output_codec_context, int *data_present, int i)
{
	/** Packet used for temporary storage. */
	...
	*/
	...
	if (*data_present) 
	{
		if (i == 0)
		{
			ff1.write((const char*)output_packet.data, output_packet.size);
			flush(ff1);
		}
			
		if ((error = /*av_write_frame*/av_interleaved_write_frame(output_format_context, &output_packet)) < 0) {//av_interleaved_write_frame
		...
		if (data_present_in_graph)
		{
			...
			for (int k = 0; k < tracks; ++k)
			{
				...
				while (1)
				{
					...
					//for (int j = 0; j < tracks; ++j)
					//{
					ret = encode_audio_frame(filt_frame, out_ifcx[k], out_iccx[k], &data_present, k);
					if (ret < 0){ std::cout << "\n 2.6"; goto end; }
					//}
...
//------------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------------
int CRTPReceive::open_output_file2(const char *filename, AVFormatContext **input_format_context, AVCodecContext *input_codec_context,
	AVFormatContext **output_format_context, AVCodecContext **output_codec_context, int i)
{
	//AVIOContext *output_io_context = NULL;
	AVStream *stream = NULL;
	AVCodec *output_codec = NULL;
	int error;
	vector<string> strRTP;
	string temp;
	//temp = "\n\nv=0\no=- 0 0 IN IP4 10.77.7.19\ns=I'll Be Gone\nc=IN IP4 10.77.7.19\nt=0 0\na=tool:libavformat 57.3.100\nm=audio 30000 RTP/AVP 97\nb=AS:705\na=rtpmap:97 PCMU/44100/2\n";
	temp = "rtp://10.77.7.19:30000";
	strRTP.push_back(temp);
	//temp = "\n\nv=0\no=- 0 0 IN IP4 10.77.7.19\ns=I'll Be Gone\nc=IN IP4 10.77.7.19\nt=0 0\na=tool:libavformat 57.3.100\nm=audio 30002 RTP/AVP 97\nb=AS:705\na=rtpmap:97 PCMU/44100/2\n";
	temp = "rtp://10.77.7.19:30002";
	strRTP.push_back(temp);
	//temp = "\n\nv=0\no=- 0 0 IN IP4 10.77.7.19\ns=I'll Be Gone\nc=IN IP4 10.77.7.19\nt=0 0\na=tool:libavformat 57.3.100\nm=audio 30004 RTP/AVP 97\nb=AS:705\na=rtpmap:97 PCMU/44100/2\n";
	temp = "rtp://10.77.7.19:30004";
	strRTP.push_back(temp);
	cout << "\n 1";
	error = avformat_alloc_output_context2(output_format_context, nullptr, "rtp", strRTP[i].c_str());
	if (error < 0) {cout << "\n ERRROR";system("pause");}
//-----------------------------------------------------------------------------------------------------------
	//** Open the output file to write to it. 
	cout << "\n 2";
	if ((error = avio_open(&(*output_format_context)->pb, strRTP[i].c_str(),
		AVIO_FLAG_WRITE)) < 0) {
		av_log(NULL, AV_LOG_ERROR, "Could not open output file '%s' (error '%s')\n",
			strRTP[i].c_str(), get_error_text(error));
		return error;
	}
	cout << "\n 3";
	//** Create a new format context for the output container format. 
	...
	cout << "\n 5";
	av_strlcpy((*output_format_context)->filename, strRTP[i].c_str(),
		sizeof((*output_format_context)->filename));
	cout << "\n 6";
	//** Find the encoder to be used by its name. 
	if (!(output_codec = avcodec_find_encoder((*input_format_context)->streams[0]->codec->codec_id/*AV_CODEC_ID_PCM_S16LE*/))) {
	...
	*output_codec_context = stream->codec;
	...
	(*output_codec_context)->channels = OUTPUT_CHANNELS;
	(*output_codec_context)->channel_layout = av_get_default_channel_layout(OUTPUT_CHANNELS);
	(*output_codec_context)->sample_rate = 8000;//input_codec_context->sample_rate;
	(*output_codec_context)->sample_fmt = AV_SAMPLE_FMT_S16;
	(*output_codec_context)->bit_rate = 8000;//input_codec_context->bit_rate;
